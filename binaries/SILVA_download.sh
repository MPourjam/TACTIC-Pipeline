...
if [[ ! -s "${parent_path}/silva-arc-16s-id95.fasta" ]]; then
    ...
    gunzip -d "${database_dir}/silva-arc-16s-id95.fasta.gz" -c > "${parent_path}/silva-arc-16s-id95.fasta" && rm "${database_dir}/silva-arc-16s-id95.fasta.gz"
else
...
if [[ ! -s "${parent_path}/silva-bac-16s-id90.fasta" ]]; then
    ...
    gunzip -d "${database_dir}/silva-bac-16s-id90.fasta.gz" -c > "${parent_path}/silva-bac-16s-id90.fasta" && rm "${database_dir}/silva-bac-16s-id90.fasta.gz"
else
